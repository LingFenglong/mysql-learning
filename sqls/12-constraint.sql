...
insert into tb_null values (1, 'licai', 'a@163.com', 8400);

select * from tb_null;

-- 在 tb_null 表中更改数据，测试非空约束的效果

update tb_null set name = null where id = 1;

-- 修改 tb_null 表，给 email 字段添加非空约束，确认非空约束设置成功

alter table tb_null modify email varchar(25) not null;

desc tb_null;

-- 删除 tb_null 表中 email 字段的非空约束，确认非空约束删除成功

alter table tb_null modify email varchar(25) null;
...
insert into cs.tb_unique values (1, 'zs', 'a@qq.com', 6000);

insert into cs.tb_unique values (1, 'zs', 'a@qq.com', 6000);

insert into cs.tb_unique values (2, 'zs', 'a@qq.com', 6000);

-- 在 tb_unique 表中插入空值数据，测试唯一值约束的效果

insert into cs.tb_unique values (null, 'zs', null, 6000);
...
add
    constraint uk_tb_unique_sal unique (salary);

-- 列级约束

alter table cs.tb_unique modify name varchar(15) unique;
...
insert into cs.tb_user values (1, 'zs', '123');

insert into cs.tb_user values (1, 'zs', '124');

insert into cs.tb_user values (1, 'ls', '123');

insert into cs.tb_user values (1, 'ls', '124');

insert into cs.tb_user values (1, 'ls', '124');

select * from cs.tb_user;

-- 在 tb_user 表中插入空值数据，测试唯一值约束的效果

insert into cs.tb_user values (null, null, null);
...
insert into cs.tb_pk1 values (null, null, null, null);

insert into cs.tb_pk1 values (1, null, null, null);

insert into cs.tb_pk1 values (2, null, null, null);
...
insert into cs.tb_pk3 values (1, 'zs', 123);

insert into cs.tb_pk3 values (1, null, 123);

insert into cs.tb_pk3 values (1, 'zs', null);

insert into cs.tb_pk3 values (1, null, null);
...
alter table cs.tb_pk4 modify id int primary key;
